...
setup_env() {
  ...
  if [ ! -z "$(jq --raw-output '.email_pass' $CONFIG_PATH)"]; then
    export KEGBOT_EMAIL_PASSWORD=$(jq --raw-output '.email_pass' $CONFIG_PATH)
  fi
  if [ ! -z "$(jq --raw-output '.email_ssl' $CONFIG_PATH)"]; then
    export KEGBOT_EMAIL_USE_SSL=$(jq --raw-output '.email_ssl' $CONFIG_PATH)
  fi
  if [ ! -z "$(jq --raw-output '.email_tls' $CONFIG_PATH)"]; then
    export KEGBOT_EMAIL_USE_TLS=$(jq --raw-output '.email_tls' $CONFIG_PATH)
  fi
